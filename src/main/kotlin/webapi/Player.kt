...
data class PlayerData(
    ...
    constructor(player: OfflinePlayer) : this(
        ...
        isOp = player.isOp,
        stats = Statistic.values().filter { it.type == Statistic.Type.UNTYPED }.associate {
            it.name to player.getStatistic(it)
        },
        itemStats = Material.values().filter { it.isItem }.associate {
            it.name to ItemStats(
        ...
        mobStats = EntityType.values().filter { it.isAlive }.associate {
            it.name to MobStats(
                killed = player.getStatistic(Statistic.KILL_ENTITY, it),
                killedBy = player.getStatistic(Statistic.ENTITY_KILLED_BY, it)
            )
        },
...
data class OnlinePlayerData(
    ...
    constructor(player: Player) : this(
        ...
        advancements = Bukkit.getServer().advancementIterator().asSequence().map { advancement ->
            advancement.key.key to Advancement(
                name = advancement.key.key,
                isDone = player.getAdvancementProgress(advancement).isDone,
                awardedCriteria = player.getAdvancementProgress(advancement).awardedCriteria,
                remainingCriteria = player.getAdvancementProgress(advancement).remainingCriteria
            )
        }.toMap(),
        attributes = Attribute.values().associate {
            it.name to player.getAttribute(it)?.value
        }
    )
}